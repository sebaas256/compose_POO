...
begin
    ...
    -- CAMBIO: Añadimos IDUsuario = 1 (El Administrador) a cada préstamo
    insert into Prestamo (IDEstudiante, IDLibro, IDUsuario, FechaSalida, FechaMaxDevolucion, FechaEntrega, Estado)
    values (
        @randomEstudiante,
        @randomLibro,
        1, -- Asignando el préstamo al usuario administrador
        @fSalida,
        @fMax,
        -- Lógica para el estado:
        case 
            when @i % 10 = 0 then null -- 10% están en MORA
            when @i % 3 = 0 then null  -- Algunos siguen ACTIVOS
            else dateadd(day, 5, @fSalida) -- El resto se DEVOLVIÓ
        end,
        case 
...
-- CAMBIO: Modifiqué el SELECT final para mostrar al usuario que atendió el préstamo
select top 10 
    E.Nombre + ' ' + E.Apellido as Estudiante,
    L.Titulo as Libro,
    U.NombreUsuario as AtendidoPor, -- Aquí vemos la trazabilidad
    P.Estado,
